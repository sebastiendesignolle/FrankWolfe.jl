...
"""
    blended_pairwise_conditional_gradient_fista(f, grad!, lmo, x0; kwargs...)
...
"""
function blended_pairwise_conditional_gradient_fista(
    f,
    grad!,
    lmo,
    x0;
    ...
    return blended_pairwise_conditional_gradient_fista(
        f,
        grad!,
        lmo,
        active_set,
        ...
        fista_interval=fista_interval,
        fista_maxiter=fista_maxiter,
        fista_printstep=fista_printstep,
        fista_accelerated=fista_accelerated,
        lazy=lazy,
...
"""
    blended_pairwise_conditional_gradient_fista(f, grad!, lmo, active_set::AbstractActiveSet; kwargs...)
...
function blended_pairwise_conditional_gradient_fista(
    ...
    p = zero(x)
    grad!(p, p) # assumes distance function to recover target point
    p .*= -1
    flatp = p[:]
...
"""
Projects a vector x on the unit simplex.
The result is stored in `v`. `u` and `cssv` are temporary vectors, same size as `x`.
"""
function projection_simplex_sort!(v::Vector{T}, x::Vector{T}, u::Vector{T}, cssv::Vector{T}; s=one(T), tol=Base.rtoldefault(T)) where {T <: AbstractFloat}
    @. v = x
    if abs(sum(x) - s) < tol && all(≥(zero(T)), x)
        return v
    end
    mx = maximum(x)
    @. v -= mx
    ...
    rho = sum(eachindex(u)) do idx
        u[idx] * idx > cssv[idx] - s
    end - 1
    theta = (cssv[rho+1] - s) / (rho + 1)
    @. v -= theta
    clamp!(v, zero(T), typemax(T))
    return v
end
...
function fista!(active_set::AbstractActiveSet, flatp::Vector{T}; maxiter=10^3, printstep=10, accelerated=true, verbose=true) where {T <: AbstractFloat}
    ...
    for j in eachindex(active_set)
        copyto!(@view(Vt[j, :]), active_set.atoms[j])
    end
    VtV = Vt * Vt'
    λ = active_set.weights
    L = maximum(eigen(Hermitian(VtV)).values)
    stepsize = inv(L)
    ...
    for t in 1:maxiter
        if accelerated
            ...
            @. gstep = γ
            # gstep -= step * V' V γ
            mul!(gstep, VtV, γ, -stepsize, 1)
            # gstep += step * V' p
            mul!(gstep, Vt, flatp, stepsize, 1)
            @. λ_prev = λ
        else
            @. gstep = λ
            # gstep -= step * V' V λ
            mul!(gstep, VtV, λ, -stepsize, 1)
            # gstep += step * V' p
            mul!(gstep, Vt, flatp, stepsize, 1)
        end
